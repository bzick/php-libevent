...
/* {{{ proto void event_buffer_flush(resource bevent, int iotype, int mode)
 */
static PHP_FUNCTION(event_buffer_flush) {
    zval *zbevent;
    long iotype;
    long mode;
    int r;
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "rll", &zbevent, &iotype, &mode) != SUCCESS) {
        return;
    }

    ZVAL_TO_BEVENT(zbevent, bevent);

    r = bufferevent_flush(bevent->bevent, iotype, mode);
    if (!r) {
        RETURN_TRUE;
    } else {
        RETURN_FALSE;
...
/* {{{ proto void event_buffer_get_length(resource bevent)
 */
static PHP_FUNCTION(event_buffer_get_length) {
    zval *zbevent;
    struct evbuffer *buffer;
    long type;
    php_bufferevent_t *bevent;

    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "rl", &zbevent, &type) != SUCCESS) {
        return;
    }
    ...
    if (type == EV_READ) {
        ...
        buffer = bufferevent_get_output(bevent->bevent);
    } else {
        RETURN_FALSE;
    }
...
static PHP_FUNCTION(event_buffer_read) {
    ...
    if (data_size == 0) {
        data_size = evbuffer_get_length(bufferevent_get_input(bevent->bevent));
    } else if (data_size < 0) {
...
static PHP_FUNCTION(event_buffer_readln) {
    zval *zbevent;
    php_bufferevent_t *bevent;
    struct evbuffer *buffer;
    long eol_style = EVBUFFER_EOL_CRLF_STRICT;
    size_t len;
    char *data;
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "r|l", &zbevent, &eol_style) == FAILURE) {
        RETURN_NULL();
    }
    ...
    data = evbuffer_readln(buffer, &len, (short) eol_style);
    if (data == NULL) {
        RETURN_FALSE;
    } else if (len == 0) {
        RETURN_EMPTY_STRING();
...
static PHP_FUNCTION(event_buffer_gets) {
    zval *zbevent;
    php_bufferevent_t *bevent;
    struct evbuffer *buffer;
...
/* {{{ proto bool event_sendfile(resource bevent, resource fd[, int offset[, int length]])
 */
static PHP_FUNCTION(event_buffer_sendfile) {
    zval *zbevent, *zfile;
    php_bufferevent_t *bevent;
    ...
    if (evbuffer_add_file(
            bufferevent_get_output(bevent->bevent),
            fd2,
            (ev_off_t) offset,
            (ev_off_t) length
            )
...
static PHP_MINIT_FUNCTION(libevent) {
    ...
    REGISTER_LONG_CONSTANT("EV_READ_WRITE", EV_READ | EV_WRITE, CONST_CS | CONST_PERSISTENT);
    ...
    REGISTER_LONG_CONSTANT("BEV_NORMAL", BEV_NORMAL, CONST_CS | CONST_PERSISTENT);
    REGISTER_LONG_CONSTANT("BEV_FLUSH", BEV_FLUSH, CONST_CS | CONST_PERSISTENT);
    REGISTER_LONG_CONSTANT("BEV_FINISHED", BEV_FINISHED, CONST_CS | CONST_PERSISTENT);

    REGISTER_LONG_CONSTANT("BEV_WITHOUT_TOKEN", BEV_WITHOUT_TOKEN, CONST_CS | CONST_PERSISTENT);
    REGISTER_LONG_CONSTANT("BEV_WITH_TOKEN", BEV_WITH_TOKEN, CONST_CS | CONST_PERSISTENT);
    REGISTER_LONG_CONSTANT("BEV_TRIM_TOKEN", BEV_TRIM_TOKEN, CONST_CS | CONST_PERSISTENT);

    REGISTER_LONG_CONSTANT("BEV_EOL_ANY", EVBUFFER_EOL_ANY, CONST_CS | CONST_PERSISTENT);
    REGISTER_LONG_CONSTANT("BEV_EOL_CRLF", EVBUFFER_EOL_CRLF, CONST_CS | CONST_PERSISTENT);
    REGISTER_LONG_CONSTANT("BEV_EOL_CRLF_STRICT", EVBUFFER_EOL_CRLF_STRICT, CONST_CS | CONST_PERSISTENT);
    REGISTER_LONG_CONSTANT("BEV_EOL_LF", EVBUFFER_EOL_LF, CONST_CS | CONST_PERSISTENT);

    REGISTER_LONG_CONSTANT("BEV_OPT_CLOSE_ON_FREE", BEV_OPT_CLOSE_ON_FREE, CONST_CS | CONST_PERSISTENT);
    REGISTER_LONG_CONSTANT("BEV_OPT_DEFER_CALLBACKS", BEV_FLUSH, CONST_CS | CONST_PERSISTENT);
    REGISTER_LONG_CONSTANT("BEV_OPT_UNLOCK_CALLBACKS", BEV_FINISHED, CONST_CS | CONST_PERSISTENT);
...
EVENT_ARGINFO
ZEND_BEGIN_ARG_INFO_EX(arginfo_event_buffer_flush, 0, 0, 3)
ZEND_ARG_INFO(0, stream)
ZEND_ARG_INFO(0, events)
...
ZEND_BEGIN_ARG_INFO_EX(arginfo_event_buffer_readln, 0, 0, 1)
ZEND_ARG_INFO(0, bevent)
ZEND_ARG_INFO(0, eol_style)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(arginfo_event_buffer_get_length, 0, 0, 1)
ZEND_ARG_INFO(0, bevent)
ZEND_ARG_INFO(0, type)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(arginfo_event_buffer_gets, 0, 0, 2)
ZEND_ARG_INFO(0, bevent)
...
ZEND_BEGIN_ARG_INFO_EX(arginfo_event_buffer_sendfile, 0, 0, 2)
ZEND_ARG_INFO(0, bevent)
ZEND_ARG_INFO(0, fd)
...
const zend_function_entry libevent_functions[] = {
    ...
    PHP_FE(event_buffer_flush, arginfo_event_buffer_flush)
    ...
    PHP_FE(event_buffer_gets, arginfo_event_buffer_gets)
    PHP_FE(event_buffer_readln, arginfo_event_buffer_readln)
    PHP_FE(event_buffer_get_length, arginfo_event_buffer_get_length)
    PHP_FE(event_buffer_sendfile, arginfo_event_buffer_sendfile)
